...
def get_dag_run_summary(context):
    ...
    if dag_run:
        ...
        for ti in task_instances:
            start_date = ti.start_date.strftime('%Y-%m-%d %H:%M:%S') if ti.start_date else 'Not started'
            end_date = ti.end_date.strftime('%Y-%m-%d %H:%M:%S') if ti.end_date else 'Not finished'
...
def send_task_email(context):
    ...
    info_warnings = []
    if task_logs:
        info_warnings = [line for line in task_logs.splitlines() if "INFO" in line or "WARNING" in line]
    ...
    if context.get("exception"):
        ...
        error_traceback = ''.join(traceback.format_tb(context.get("exception").__traceback__)) if context.get("exception") else "No traceback available"
