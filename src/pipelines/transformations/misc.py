...
class ParquetDatabaseHandler:
                    ...
                    if security_type_dir.is_dir():
                        ...
                        for timeframe_dir in security_type_dir.iterdir():
                            if timeframe_dir.is_dir():
                                ...
                                self.timeframes[security_key].add(
                                    timeframe
                                )
                                timeframe_key = (
                                    exchange,
                                ...
                                for file_path in timeframe_dir.glob(
                                    "*.parquet"
                                ):
    ...
    def create_security_type_directory(
        self, exchange: str, security_type: str
    ) -> Path:
        exchange = self._sanitize_name(exchange)
        ...
        if not security_type_dir.exists():
            ...
            logger.info(
                f"Created security type directory: {security_type_dir}"
            )
    ...
    def create_timeframe_directory(
        ...
        security_type_dir = self.create_security_type_directory(
        ...
        if not timeframe_dir.exists():
            ...
            logger.info(f"Created timeframe directory: {timeframe_dir}")
        ...
        if file_path.exists():
            try:
                ...
                if "datetime" in combined_data.columns:
                    ...
                    combined_data = combined_data.sort("date").unique(
                        subset=["date"]
                    )
                else:
    ...
    def delete_symbol_data(
        ...
        if file_path.exists():
            ...
            timeframe_key = (exchange, security_type, timeframe)
            if (
                timeframe_key in self.symbols
                and symbol in self.symbols[timeframe_key]
            ):
    ...
    def delete_timeframe(
        ...
        timeframe_dir = (
            self.base_path / exchange / security_type / timeframe
        )
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        symbol_count = sum(
            len(symbols) for symbols in self.symbols.values()
        )
    ...
    def search_symbols(
        ...
        for exch in exchanges_to_search:
            ...
            security_types_to_search = (
                [security_type]
                if security_type
                else self.security_types.get(exch, [])
            )
            ...
            for sec_type in security_types_to_search:
                ...
                timeframes_to_search = (
                    [timeframe]
                    if timeframe
                    else self.timeframes.get(security_key, [])
                )
