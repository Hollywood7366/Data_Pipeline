...
def get_tick_historical(
    ...
    try:
        ...
        for sym in symbols:
            ...
            current_start_date = adjusted_start_dt.strftime(
                "%Y%m%d 000000"
            )
            ...
            if data:
                ...
                if data and not data.isspace():
                    ...
                    logger.info(
                        f"Successfully processed {record_count} tick records for {sym}"
                    )
                    ...
                    logger.warning(
                        f"No valid tick data received for {sym}"
                    )
                    run_async_task(
                        extraction_manager.record_extraction(
                            ticker=sym,
        ...
        logger.info(
            f"Tick data extraction completed. Processed {len(successful_tickers)} symbols successfully."
        )
