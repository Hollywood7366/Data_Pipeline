...
class ExtractionManager:
    ...
    async def record_extraction(
        ...
        try:
            if successful:
                ...
                logger.info(f"Recorded successful extraction for {ticker} with {record_count} records")
            else:
                logger.info(f"Skipped recording failed extraction for {ticker}")
    ...
    async def get_ticker_extractions(self, ticker: str) -> List[Any]:
        try:
            ...
            logger.error(f"Error retrieving extraction records for {ticker}: {e}")
            return []
    ...
    async def get_successful_extractions(self) -> List[Any]:
        try:
            ...
            logger.error(f"Error retrieving successful extraction records: {e}")
            return []
    ...
    async def get_latest_extraction(self, ticker: str) -> Any:
        try:
            ...
            if records:
                return sorted(records, key=lambda x: x.extraction_date, reverse=True)[0]
            return None
        except Exception as e:
            logger.error(f"Error retrieving latest extraction for {ticker}: {e}")
            return None
    
    def should_process_ticker(self, sym, start_dt, end_dt, interval) -> bool:
        latest_extraction = run_async_task(self.get_latest_extraction(sym))
        
        if latest_extraction:
            same_dates = (latest_extraction.start_date.date() == start_dt.date() and 
                        latest_extraction.end_date.date() == end_dt.date())
            same_interval = latest_extraction.interval == interval
            
            if same_dates and same_interval and latest_extraction.successful:
                logger.info(f"Skipping {sym}: already successfully processed for the requested date range")
                return False
