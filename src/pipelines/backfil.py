...
def is_symbol_already_processed(symbol, start_date, end_date):
    metadata_file = os.path.join(
        os.getenv("STORAGE_PATH", "data/market_data"),
        "backfill_metadata.txt",
    )
    ...
    try:
        ...
        logger.error(
            f"Error checking if symbol {symbol} was processed: {e}"
        )
...
def identify_and_backfill_missing_dates(security_type):
    if not should_run_security_type(security_type):
        logger.info(
            f"Skipping {security_type} as it's not enabled in GET_THIS_TYPE"
        )
        return
    ...
    logger.info(
        f"Found {security_type_df.height} symbols for security_type: {security_type}"
    )
    ...
    for row in security_type_df.iter_rows(named=True):
        ...
        try:
            try:
                ...
                symbol_data = db_handler.load_data(
                    exchange, security_type, interval, symbol
                )
                ...
                if (
                    ...
                    or date_col not in symbol_data.columns
                ):
                ...
                if (
                    ...
                    if not dates:
                        ...
                        start_ranges = [
                            (min_date, dates[0] - timedelta(days=1))
                        ]
                        end_ranges = [
                            (dates[-1] + timedelta(days=1), max_date)
                        ]
                        ...
                        all_ranges = (
                            start_ranges + middle_ranges + end_ranges
                        )
    ...
    logger.info(
        f"Found {len(symbols_to_backfill)} {security_type} symbol ranges to backfill."
    )
    ...
    for i, (start_date, end_date) in enumerate(missing_date_ranges):
        ...
        logger.info(
            f"Backfilling {security_type} symbol {symbol} from {start_date} to {end_date}"
        )
        ...
        try:
            ...
            if is_symbol_already_processed(symbol, start_date, end_date):
                logger.info(
                    f"Symbol {symbol} already processed for date range {start_date} to {end_date}, skipping..."
                )
                continue
            ...
            logger.info(
                f"Successfully backfilled {security_type} symbol {symbol} for date range {start_date} to {end_date}"
            )
            mark_symbol_as_processed(symbol, start_date, end_date)

        except Exception as e:
            logger.error(
                f"Failed to backfill {security_type} symbol {symbol} for date range {start_date} to {end_date}: {e}"
            )
            continue

    logger.info(
        f"Completed backfill of {len(symbols_to_backfill)} {security_type} symbol ranges"
    )
