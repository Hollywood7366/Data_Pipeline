...
dag = DAG(
    dag_id="DEDUPLICATION_PARQUET_FILES_V1.0.1",
    ...
    catchup=False,
    tags=["parquet", "deduplication", "data_cleaning"],
)


def find_parquet_files(storage_dir):
    parquet_files = glob(os.path.join(storage_dir, "**", "*.parquet"), recursive=True)
...
def deduplicate_parquet_file(file_path):
    try:
        ...
        if df.is_empty() or "DateTime" not in df.columns:
            logger.info(f"Skipping {file_path}: Empty or missing DateTime column")
            return {
                "file": file_path,
                "status": "skipped",
        ...
        if new_row_count < original_row_count:
            ...
            logger.info(f"Deduplicated {file_path}: Removed {original_row_count - new_row_count} duplicate rows")
        else:
...
def process_all_parquet_files(**kwargs):
    ...
    total_duplicates_removed = sum(r.get("duplicates_removed", 0) for r in processed)
    ...
    logger.info(f"- Total duplicate rows removed: {total_duplicates_removed}")
