...
class ParquetDatabaseHandler:
    ...
    def __init__(self, base_path: str):
        ...
        self.exchanges = set()
        self.security_types = {}  # Dictionary mapping exchanges to sets of security types
        self.timeframes = {}  # Dictionary mapping (exchange, security_type) tuples to sets of timeframes
        self.symbols = {}  # Dictionary mapping (exchange, security_type, timeframe) tuples to sets of symbols
    ...
    def create_timeframe_directory(self, exchange: str, security_type: str, timeframe: str) -> Path:
        """
    ...
    def save_data(self, exchange: str, security_type: str, timeframe: str, 
                  symbol: str, data: pd.DataFrame) -> Path:
        """
        ...
        timeframe_dir = self.create_timeframe_directory(exchange, security_type, timeframe)
    ...
    def load_data(self, exchange: str, security_type: str, timeframe: str, 
        ...
        file_path = self.base_path / exchange / security_type / timeframe / f"{symbol}.parquet"
        
        if not file_path.exists():
            raise FileNotFoundError(f"No data found for {symbol} ({timeframe}/{security_type}) on {exchange}")
        
        return pd.read_parquet(file_path)
    
    def append_data(self, exchange: str, security_type: str, timeframe: str,
                  symbol: str, new_data: pd.DataFrame) -> Path:
        """
        ...
        if file_path.exists():
            try:
                ...
                if 'datetime' in combined_data.columns:
                    combined_data = combined_data.sort_values('datetime').drop_duplicates()
    ...
    def list_symbols(self, exchange: str, security_type: str, timeframe: str) -> List[str]:
        """
    ...
    def delete_symbol_data(self, exchange: str, security_type: str, timeframe: str, symbol: str) -> bool:
        """
    ...
    def delete_timeframe(self, exchange: str, security_type: str, timeframe: str) -> bool:
        """
        ...
        if timeframe_dir.exists():
            ...
            security_key = (exchange, security_type)
            if security_key in self.timeframes and timeframe in self.timeframes[security_key]:
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        security_type_count = sum(len(sec_types) for sec_types in self.security_types.values())
        timeframe_count = sum(len(timeframes) for timeframes in self.timeframes.values())
    ...
    def search_symbols(self, pattern: str, exchange: Optional[str] = None, 
                      security_type: Optional[str] = None, 
                      timeframe: Optional[str] = None) -> List[Tuple[str, str, str, str]]:
        """
        ...
        for exch in exchanges_to_search:
            ...
            security_types_to_search = [security_type] if security_type else self.security_types.get(exch, [])
            ...
            for sec_type in security_types_to_search:
                ...
                timeframes_to_search = [timeframe] if timeframe else self.timeframes.get(security_key, [])
...
if __name__ == "__main__":
    ...
    for result in search_results:
        print(f"Exchange: {result[0]}, Security Type: {result[1]}, Timeframe: {result[2]}, Symbol: {result[3]}")
